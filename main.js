(function () {
  ...
  if (spark) {
    ...
    function onMove(e) {
      ...
      tip.textContent = 'Day ' + (best + 1) + ' · SOFA ' + vals[best];
  ...
  document.querySelectorAll('[data-reveal-group]').forEach(function (group) {
    ...
    var n = kids.length;
    // Compress stagger so the whole group never exceeds ~400ms
    var scale = n > 6 ? 6 / n : 1;
  ...
  if (reduceMotion || !('IntersectionObserver' in window)) {
    ...
    // Counters that are not themselves inside a data-reveal element
  ...
  function onScroll() {
    ...
    // Safety net: anything scrolled fully past the viewport is shown,
    // so a very fast flick can never leave content hidden.
